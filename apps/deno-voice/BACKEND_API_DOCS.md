...
### 2.0 Mobile-compatible contract

Sau khi merge `main`, mobile/Nest contract chính trong `docs/PLAN.md` dùng các endpoint:
...
Service Deno expose thêm các endpoint trên để chạy như một service riêng mà vẫn tương thích client mới. Các endpoint cũ `/session`, `/turn`, `/conversation` vẫn giữ cho web demo và test cũ.
